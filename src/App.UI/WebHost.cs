    ...
    private static void DoWindowAction(Window w, string action)
    {
        switch (action)
        {
            ...
            default:
                ...
                else if (action.StartsWith("corners:", StringComparison.Ordinal)
                         && double.TryParse(action.Substring(8), System.Globalization.NumberStyles.Float,
                                            System.Globalization.CultureInfo.InvariantCulture, out double rad))
                    WindowRounding.Set(w, rad);
                break;
        }
    }
