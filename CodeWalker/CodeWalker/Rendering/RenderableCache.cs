...
namespace CodeWalker.Rendering
{
    public class RenderableCache
    {
        ...
        public int MaxItemsPerLoop = 8; //Epic RPF: was 1 - higher = much faster GPU upload (models appear sooner)
        ...
        //Epic RPF: change the GPU geometry/texture cache budgets at runtime (bytes).
        public void SetCacheLimits(long geometry, long texture, long bound)
        {
            if (geometry > 0) renderables.CacheLimit = geometry;
            if (texture > 0) textures.CacheLimit = texture;
            if (bound > 0) boundcomps.CacheLimit = bound;
        }
        public long GeometryCacheLimit { get { return renderables.CacheLimit; } }
        public long TextureCacheLimit { get { return textures.CacheLimit; } }
