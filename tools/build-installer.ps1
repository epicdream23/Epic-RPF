...
Write-Host "== publishing rpfcli (powers the in-app terminal) =="
# Self-contained into a cli\ subfolder so it runs on machines without .NET, and its runtime
# never collides with the app's own. The installer bundles dist\publish\* recursively.
dotnet publish (Join-Path $root "src\App.Cli\App.Cli.csproj") -c Release -r win-x64 `
  --self-contained true -p:PublishSingleFile=false -o (Join-Path $root "dist\publish\cli") -v quiet
if ($LASTEXITCODE -ne 0) { Write-Host "   ! rpfcli publish failed - the in-app terminal will be unavailable" }
else { Invoke-Sign (Join-Path $root "dist\publish\cli\rpfcli.exe") }
